...
public class PostsRepositoryImpl implements PostsRepository {
    ...
    private final static String SQL_FIND_FEED = "select p2.id as posts_id, author_id, created_at, content, p.id as users_id, first_name, last_name, age, password_hash, email, avatar_id\n" +
            "from follows\n" +
            "         right join users p on p.id = follows.following_id\n" +
            "         right join posts p2 on p.id = p2.author_id\n" +
            "where profile_id = ?  or author_id = ? \n" +
            "order by created_at desc;";
    private final static String SQL_DELETE = "delete from posts where id = ?;";
    ...
    @Override
    public List<Post> findFeed(Long profile_id) {
        return jdbcTemplate.query(SQL_FIND_FEED, rowMapper, profile_id, profile_id);
    }
